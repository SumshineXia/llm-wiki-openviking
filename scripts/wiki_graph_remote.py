...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--include-root-pages", action="store_true", help="Include index/overview/log in the graph")
    parser.add_argument("--dry-run", action="store_true", help="Do not write graph artifacts")
    ...
    try:
        with OVFSClient(config) as client:
            graph_data = build_nodes_and_edges(
                client=client,
                kb_root=kb_root,
                include_root_pages=args.include_root_pages,
            )

            node_count = len(graph_data["nodes"])
            edge_count = len(graph_data["edges"])

            graph_json_uri = kb_root + "graph/graph.json"
            graph_html_uri = kb_root + "graph/graph.html"
