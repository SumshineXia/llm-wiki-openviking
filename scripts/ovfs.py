...
class OVFSConfig:
    ...
    def load(cls) -> "OVFSConfig":
        ...
        - OPENVIKING_TIMEOUT
        """
        env_url = os.getenv("OPENVIKING_URL")
        env_api_key = os.getenv("OPENVIKING_API_KEY")
        ...
        if conf_path.exists():
            try:
                ...
                file_timeout = data.get("timeout")
            except Exception as exc:
                raise OVFSError(f"Failed to parse {conf_path}: {exc}") from exc

        url = env_url or file_url or "http://localhost:1933"
        api_key = env_api_key or file_api_key

        timeout_value: float = 30.0
        raw_timeout = env_timeout or file_timeout
        if raw_timeout is not None:
            try:
                timeout_value = float(raw_timeout)
            except ValueError as exc:
                raise OVFSError(f"Invalid timeout value: {raw_timeout}") from exc
